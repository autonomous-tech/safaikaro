...
The queue is a file: tools/weekly/queue.json, a list of {"id","type","page","query","play","reason","added","status":"queued|shipped|dropped|needs_human"}. It carries no metrics and is committed with the PR, so it persists between runs.
 1. Read queue.json. Items still "queued" from last week are executed FIRST this run (unless the new data says the play no longer makes sense; then mark it dropped with the reason).
 2. For every gaining, slipping and striking-distance row in report_data.gsc and every cro.leaks row, decide the play (the defaults are in send.py play_for; override when the data says otherwise). If the play is something the routine can do on the site (title, meta, H1, section, FAQ pair, internal links, definition block, price anchor, CTA placement, freshness stamp, area page section, new post), it goes in the queue and is executed this run. If it needs a human (links, citations, GBP, ops decisions), it goes to insights.suggestions with an owner.
 3. Rebuild the rest of the queue from report_data and config, scored impressions x position gap x intent:
